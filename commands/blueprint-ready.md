...
- **同步藍圖狀態**：第一個階段開始時，`Draft` → `In Progress`

**時機 2：完成階段**
- 偵測：「完成了」、「階段 N 完成了」
- 行動：確認後更新 `In Progress` → `Done` + 記錄完成內容
- **同步藍圖狀態**：所有階段完成時，`In Progress` → `Completed`
...
**藍圖狀態同步規則**：
- 第一個階段從 Pending → In Progress：藍圖 `Draft` → `In Progress`
- 所有階段都為 Done：藍圖 `In Progress` → `Completed`
- 修改任何階段狀態後都應檢查並同步藍圖狀態
