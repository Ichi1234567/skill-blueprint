...
- **同時建立藍圖**：兩個會話都檢測到 `blueprint.md` 存在，都嘗試暫停舊藍圖 → 檔案可能覆蓋或遺失
- **同時暫停和廢棄**：一個會話暫停，另一個會話廢棄 → 檔案可能移動到錯誤的目錄
- **恢復與建立衝突**：一個會話恢復藍圖，另一個會話建立新藍圖 → `blueprint.md` 可能被意外覆蓋
...
**為什麼不鎖定 `blueprint.md`？**
- `blueprint.md` 可能不存在（新專案、剛暫停/廢棄後）
- `blueprint.md` 會被移動（暫停、廢棄、歸檔時）
- 專用鎖文件更穩定、語義更清楚
...
會。鎖定機制使用 trap 機制，當程序終止時（包括崩潰），會自動呼叫 release_lock。此外，stale lock 檢測機制會在下次取得鎖時自動清理已終止 process 的鎖。

### 可以跨機器使用這個鎖嗎？

不行。此鎖定機制是本機鎖，只能防止同一台機器上的競爭條件。

如果在 NFS 或其他網路檔案系統上，需要使用其他鎖定機制。
