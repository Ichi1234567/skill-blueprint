...
修改檔案時避免重複確認，提升效率並減少 token 使用。

### 規劃後再執行

**原則**：
1. **先讀取檔案**：使用 Read 完整讀取目標檔案
2. **規劃所有修改**：列出所有需要修改的位置
3. **一次性執行**：單一訊息包含所有 Edit 呼叫

**規劃範例**：
```
blueprint-ready.md 需要修改：
1. 更新階段狀態邏輯 → 加入藍圖狀態同步
2. 輸出格式 → 加入藍圖狀態顯示
3. 建議行動 → 加入狀態檢查提示
```
...
1. **避免逐步修改**：不要改一個確認一次
2. **完整規劃**：修改前思考清楚所有變更點
3. **平行執行**：單一訊息包含所有 Edit 呼叫
4. **驗證唯一性**：確保 old_string 唯一
5. **保持完整性**：包含足夠上下文
