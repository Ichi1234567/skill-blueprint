...
5. **判斷下一步**
   ...
   **建議行動**（選擇適合的方式）：
   ...
   ◆ 選項 2：建立追蹤任務
   如果需要在任務管理系統中追蹤，可以建立 beads issue：
   ...
   💡 我會自動把 beads ID 記錄到藍圖的「關聯資訊」中

   ◆ 選項 3：詳細規劃（如果不確定怎麼做）
   如果對技術路徑不太確定，可以用詳細規格工具細化：
   ...
   **建議行動**（選擇適合的方式）：
   ...
   ◆ 選項 2：建立追蹤任務（如果需要拆分小任務）
   可以建立 beads issue 來追蹤，或進一步拆分成更小的子任務：

   回覆「幫我建立階段 [N] 的任務」

   💡 我會自動把 beads ID 記錄到藍圖中

   ◆ 選項 3：詳細規劃（如果有不確定的地方）
   如果對某些部分不太確定，可以先建立詳細規格：
   ...
   **建議行動**（選擇適合的方式）：
   ...
   ◆ 選項 2：拆分任務追蹤（如果要分階段實作）
   建立 beads issue 並拆分成多個子任務：

   回覆「幫我為階段 [N] 建立任務並拆分」

   💡 我會協助你拆分成合理的子任務，並記錄所有 beads ID

   ◆ 選項 3：直接開始（需要謹慎）
   如果你對這個階段非常熟悉，也可以直接開始：
...
2. **更新 issue 狀態**（可選）：
   ...
   關聯的 beads issue (beads-123) 目前狀態：open

   要更新為 in_progress 嗎？

   選項：
   A. 更新 issue 狀態為 in_progress
   B. 保持原狀
   ```

3. **執行更新**：
   - 如果選擇 A：
     - 使用 Bash 執行 `bd update beads-123 --status=in_progress`
